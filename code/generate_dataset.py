...
for idx in range(0, 30):
    fw = open(os.path.join('../data/h2o-3/text-davinci-003/data_' + format(idx) + '_5_25.jsonl'), 'w')
    bleu_q = []
    rouge_q = []
    ...
    for i in range(25):
        ...
        for example_idx in selected_examples:
            prompt = prompt + examples[example_idx].source + description_dict[0] + examples[example_idx].target + '\n'
        ...
        fw.write(format(idx) + " " + format(selected_examples) + " " + format(bleu) + " " + format(rouge) + "\n")
        bleu_q.append(bleu)
        rouge_q.append(rouge)
        time.sleep(20)

    fw.write("total: " + format(np.average(np.array(bleu_q))) + " " + format(np.average(np.array(rouge_q))) + "\n")
    fw.close()
